function DSC_report(ctp_movie, brain_mask, savepath)
    ...
    brain_mask_volume = double(niftiread(brain_mask));
    ...
    test_mask = brain_mask_volume; %reshape(brain_mask_volume, [R, V, S]);
    ...
    %         TE         (Echo time)
    %         TR         (Repetition time)
    %         mask       (3D matrix with brain's mask)
    % Output  cbv        (3D matrix with standard rCBV values)
    %         cbf        (struct with 3D matrices of rCBF values for
    %                     each method selected)
    %         mtt        (struct with 3D matrices of MTT values for
    %                     each method selected)
    %         cbv_lc     (3D matrix with leackage corrected rCBV values)
    %         ttp        (3D matrix with leackage corrected Time to Peak
    %                     values)
    %         mask       (3D matrix with computed mask)
    %         aif        (struct with AIF extracted with clustering
    %                     algorithm)
    %         conc       (4D matrix with pseudo-concentration values)
    %         s0         (3D matrix with S0 estimates from pre-contrast
    %                     images)
    
    [cbv,cbf,mtt,cbv_lc,ttp,tmax,mask,aif,conc,s0]=DSC_mri_core(test_volume,TE,TR,test_mask);
