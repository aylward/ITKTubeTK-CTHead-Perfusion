function [res_svd, tmax_svd]=DSC_mri_SVD(conc,aif,mask,options)
    ...
    for c=1:options.nC
        for s=1:options.nS
            if mask(r,c,s)
                ...
                vettConc=reshape(conc(r,c,s,:),options.nT,1); % compute for each voxel
...
tmax_svd = zeros(options.nR,options.nC,options.nS);
for r=1:options.nR
    for c=1:options.nC
        for s=1:options.nS
            [argvalue, argmax] = max(res_svd.residual(r, c, s, :));
             tmax_svd(r, c, s) = argmax;
        end
    end
end
