...
if options.module == 'CTP'
    fprintf('CTP Concentration Convertion');
    [S0map,bolus]=DSC_ct_S0(volumes,mask,options);
else
    fprintf('MRP Concentration Convertion');
...
for t=1:options.nT
    ...
    if options.module == 'MRP'
        step1=volumes(ind+k*(t-1))./S0map(ind);
        step2=step1.*(step1<1);
        step3=step2+(step2==0);
        conc(ind+k*(t-1))=-(options.par.kvoi/options.te).*log(step1); % relation between MR signal and concentration
    else
        conc(ind+k*(t-1))= volumes(ind+k*(t-1)) - S0map(ind); % relation between CT signal and concentration
    end
end
...
for s=1:options.nS
    ...
    for t=1:options.nT
        ...
        % compute mean only in brain region
        mean_signal(s, t) = mean(mean(nonzeros(volumes(:, :, s, t))));
...
start_s = 1;
while flag
    if isnan(mean(mean_signal(start_s, :)))
        start_s = start_s + 1;
    else
        flag = false;
    end
...
for s=start_s:options.nS
    
    % 1.2) calcolo dell'istante di iniezione del bolo
    ciclo=true;
    pos=nSamplesMin;
    while ciclo
        mean_val=mean(mean_signal(s,1:pos+1));
        if abs((mean_val-mean_signal(s,pos+1))/mean_val)<thresh % find all time points before injection to calc s0 
            pos=pos+1;
        else
            ciclo=false;
        ...
        if pos==nSamplesMax % at most 3 time points for calc s0
            ciclo=false;
            pos=pos-1;
        end
    end
...
%%
function [S0map,bolus]=DSC_ct_S0(volumes,mask,options)
%Funzione del pacchetto DSC_mri - DSC_mri_S0
%Autore: Castellaro Marco - Universit� di Padova - DEI
...
thresh=options.S0.thresh;
for t = 1:options.nT
    sig_avg(t) = mean(mean(mean(volumes(:, :, :, t),'omitnan')));
end
thresh = thresh * (max(sig_avg) - min(sig_avg));
...
for s=start_s:options.nS
    ...
    while ciclo
        ...
        if pos==nSamplesMax | pos+1>length(mean_signal(1,:)) % at most 3 time points for calc s0
            ciclo=false;
            pos=pos-1;
        end
    end
