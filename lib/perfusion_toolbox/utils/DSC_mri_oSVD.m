function [res_osvd, tmax_osvd]=DSC_mri_oSVD(conc,aif,mask,options)
...
tmax_osvd = zeros(options.nR,options.nC,options.nS);
for r=1:options.nR
    for c=1:options.nC
        for s=1:options.nS
            [argvalue, argmax] = max(res_osvd.residual(r, c, s, :));
            tmax_osvd(r, c, s) = argmax;
        end
    end
end
