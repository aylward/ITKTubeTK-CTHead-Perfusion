...
[prob,intensity]=hist(volume_sum(1:options.nR*options.nC*options.nS),nbin); % prob: presence frequency; intensity: measured signal intensity
...
%    [maxValue,maxPos]=max(prob(maxPos2:end));
%    if (maxPos~=1)||(maxPos2==length(prob))
%        exitFlag=false;
...
for s=1:options.nS
    ...
    if (options.display > 2)||((options.display >1) && (s == round(0.5*options.nS))) % display mask at the middle slice
        hf_mask(s)=figure(); 
        ...
        B = bwboundaries(temp,4); % plot mask boundary with green line
        hold on
        for k = 1:length(B)
            boundary = B{k};
