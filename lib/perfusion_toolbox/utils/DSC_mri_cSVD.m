function [res_csvd, tmax_csvd]=DSC_mri_cSVD(conc,aif,mask,options)
...
tmax_csvd = zeros(options.nR,options.nC,options.nS);
for r=1:options.nR
    for c=1:options.nC
        for s=1:options.nS
            [argvalue, argmax] = max(res_csvd.residual(r, c, s, :));
            tmax_csvd(r, c, s) = argmax;
        end
    end
end
