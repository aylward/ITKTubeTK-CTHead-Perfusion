...
%Selezione slice AIF
if ( options.aif.nSlice <= 0 )||( options.aif.nSlice > options.nS ) % if no prefixed slice for aif selection
...
centro(1)=0.6*(minC+maxC); % Coordinata X del centro (calcolata sulle colonne)
...
%sprintf('%d', length(pesi))
%sprintf('%d', length(AIFconc))
