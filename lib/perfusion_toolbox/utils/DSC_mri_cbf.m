function [cbf, tmax]=DSC_mri_cbf(conc,aif,mask,options)
...
for alg=1:size(options.deconv.method,1)
    switch options.deconv.method{alg,:}
        case method{1,:} %SVD
            [cbf.svd, tmax.svd]=DSC_mri_SVD(conc,aif,mask.data,options);
        case method{2,:} %cSVD
            [cbf.csvd, tmax.csvd]=DSC_mri_cSVD(conc,aif,mask.data,options);
        case method{3,:} %oSVD          
            [cbf.osvd, tmax.osvd]=DSC_mri_oSVD(conc,aif,mask.data,options);
